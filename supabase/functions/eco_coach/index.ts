    ...
    if (needsGeneralGen) {
      ...
      if (!insertError && placeholder) {
        try {
          ...
          const footprint = logs?.reduce((sum, log) =>
            sum + log.total_co2e, 0) || 0;

          const prompt =
            `You are CarbonSense. Do not introduce yourself. Act as a companion that gives user's total historical footprint. The total footprint is ${
              footprint.toFixed(2)
      ...
      if (!insertError && placeholder) {
        try {
          ...
          const { data: monthLogs } = await supabase
            .from("activity_logs").select("total_co2e").eq("user_id", user_id)
            .gte("logged_at", startDate).lte("logged_at", endDate);

          const monthFootprint = monthLogs?.reduce((sum, log) =>
            sum + log.total_co2e, 0) || 0;
          const monthNames = [
            "January",
            "February",
          ...
          const prompt =
            `You are CarbonSense, but do not introduce yourself. Act as a companion or an automated reminder. The month of ${
              monthNames[lastMonthIndex]
            } just ended. The user emitted a total of ${
              monthFootprint.toFixed(2)
            } kg CO2e. Provide a 2-sentence summary of their performance for that month, and suggest a goal for the new month.`;
