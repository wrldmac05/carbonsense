...
import 'package:carbonsense/features/analytics/analytics_providers.dart'; 
...
@override
  void dispose() {
    _weeklyCountdownTimer?.cancel(); // ✂️ Stop the background clock when page closes
    super.dispose();
  }
  ...
  void _updateCountdownString() {
    ...
    if (daysUntilMonday <= 0) {
      daysUntilMonday += 7; // If it's Monday today, the next reset is exactly 7 days away
    }
  ...
  Future<void> _fetchOrGenerateTasks() async {
    ...
    if (response.isEmpty) {
      ...
      final firstTaskTimestamp = response.first['created_at']; // or 'assigned_at'
  ...
  Future<void> _calculateNetFootprint() async {
    ...
    final totalSaved = savedResponse.fold<double>(0.0, (sum, item) {
       ...
       if (taskData != null) return sum + double.parse(taskData['co2_saved_estimate'].toString());
       return sum;
    });
 ...
 // 📊 FETCH RAW MONTHLY DATA FOR THE MODAL (Upgraded with relational join)
  ...
  Future<void> _updateTaskStatus(String userTaskId, bool isCompleted) async {
    try {
      ...
      final taskIndex = _tasks.indexWhere((t) => t['user_task_id'] == userTaskId);
      if (taskIndex != -1) setState(() => _tasks[taskIndex]['is_completed'] = !isCompleted);
  ...
  Future<void> _updateOnboardingTask(String column, bool currentValue) async {
    ...
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to save progress.')));
    }
  }
 ...
 // 🚀 LIFESTYLE SELECTION SHEET (FIXED FOR OVERFLOW)
  void _openLifestyleCustomizationSheet() {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true, // 👈 1. Allows the sheet to expand dynamically beyond default limits
      backgroundColor: Colors.transparent,
      builder: (BuildContext sheetContext) {
        return Container(
          decoration: const BoxDecoration(
          ...
          child: SafeArea(
            child: SingleChildScrollView( // 👈 2. Makes the layout scrollable on smaller mobile displays
              physics: const BouncingScrollPhysics(),
              child: Column(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => showQuickStartGuideDialog(context), // 🔓 Opens instantly as reference tool!
          ),
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const Text("Quick Log", style: TextStyle(fontWeight: FontWeight.w900, fontSize: 18, color: Colors.black87)),
  ...
  Widget _buildHeader() {
    return Column(
      ...
      children: [
        const Text(
          "WELCOME BACK", 
          style: TextStyle(color: AppTheme.primaryColor, fontWeight: FontWeight.w900, fontSize: 12, letterSpacing: 2.0),
        ),
        ...
        children: [
          const Row(
            children: [
              ...
              SizedBox(width: 8),
              Text("GETTING STARTED", style: TextStyle(color: AppTheme.primaryColor, fontWeight: FontWeight.w900, fontSize: 11, letterSpacing: 1.2)),
            ],
          ),
          const SizedBox(height: 16),
          ...
          _buildOnboardingTile(
            ...
            () => ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Navigate to your Profile settings screen to upload a picture!'))
            ),
          ),
          ...
          _buildOnboardingTile(
            ...
            _obFirstLog, 
            () => ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Use the Quick Log strips or Activity screens to input telemetry.'))
            ),
          ),
        ],
      ),
 ...
 Widget _buildOnboardingTile(String title, bool isDone, VoidCallback onTap) {
    return InkWell(
      onTap: isDone ? null : onTap, // If it's already completed, make it static and unclickable
      borderRadius: BorderRadius.circular(12),
      child: Padding(
    ...
    return GestureDetector(
      onTap: () {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => LogActivityScreen(category: categoryKey), // 🚀 Passes directly into your constructor
          ),
        ).then((_) {
          // Automatically re-calculates the dashboard's circular progress metrics 
          // when the user is sent back after a successful submission!
          _initializeDashboard(); 
        });
      },
      child: Column(
  ...
  Widget _buildModernTasksList() {
    if (_tasks.isEmpty) {
      return const Center(
        child: Padding(
          ...
          child: Text("All tasks completed or none generated! Tap refresh.", style: TextStyle(color: Colors.grey, fontStyle: FontStyle.italic)),
        ),
      );
    }
    ...
    if (filteredTasks.isEmpty) {
      return Center(
        child: Padding(
          padding: const EdgeInsets.all(24.0),
          child: Text('No ongoing tasks under the "$_selectedPreset" focus.', style: const TextStyle(color: Colors.grey, fontStyle: FontStyle.italic, fontSize: 13)),
        ),
      );
    }
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: isCompleted ? AppTheme.primaryColor.withOpacity(0.05) : Colors.white,
            borderRadius: BorderRadius.circular(24),
            border: Border.all(color: isCompleted ? AppTheme.primaryColor.withOpacity(0.3) : Colors.transparent),
            boxShadow: isCompleted ? [] : [BoxShadow(color: AppTheme.primaryColor.withOpacity(0.06), blurRadius: 15, offset: const Offset(0, 8))],
          ),
          child: CheckboxListTile(
            ...
            subtitle: Padding(
              ...
              child: Text('TIER: ${tier.toUpperCase()}', style: TextStyle(color: AppTheme.primaryColor.withOpacity(0.8), fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 1)),
            ),
            value: isCompleted,
