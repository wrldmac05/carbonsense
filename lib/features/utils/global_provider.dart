...
final userTasksStreamProvider = StreamProvider.autoDispose<List<Map<String, dynamic>>>((ref) {
  final userId = Supabase.instance.client.auth.currentUser?.id;
  ...
  return Supabase.instance.client.from('user_tasks').stream(primaryKey: ['user_task_id']).eq('user_id', userId).asyncMap((tasksData) async {
    ...
    for (var task in tasksData) {
      ...
      if (taskId != null) {
        final dictRes = await Supabase.instance.client.from('tasks_dictionary').select().eq('task_id', taskId).maybeSingle();
        if (dictRes != null) {
          dictData = dictRes;
        }
      }
