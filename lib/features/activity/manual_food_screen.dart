...
class _ManualFoodLogScreenState extends State<ManualFoodLogScreen> {
  ...
  void _addIngredient() {
    ...
    // Input sanitization: letters, spaces, and hyphens only
  ...
  Future<void> _submitManualLog() async {
    ...
    try {
      ...
      final rawWeight = double.tryParse(_weightController.text.trim().replaceAll(',', '.')) ?? 0.0;
      final safeWeight = rawWeight.clamp(10.0, 3000.0);

      final rawBaseCo2 = _selectedCategory!['co2_per_unit'];
      final double baseCo2 = rawBaseCo2 is num ? rawBaseCo2.toDouble() : 0.0;
      final totalCo2e = ((safeWeight / 500.0) * baseCo2).clamp(0.0, 500.0);

      final rawFoodName = _foodNameController.text.trim();
      final foodName = rawFoodName.length > 80 ? rawFoodName.substring(0, 80) : rawFoodName;

      await Supabase.instance.client.from('activity_logs').insert({
        'user_id': user.id,
        'factor_id': _selectedCategory!['factor_id'],
        'food_name': foodName,
        'input_value': double.parse(safeWeight.toStringAsFixed(2)),
      ...
      final completedMissions = await MissionEngine.evaluateTelemetry(
        userId: user.id,
        category: 'Diet',
        activityName: _selectedCategory!['name'],
        isMeatless: _selectedCategory!['is_meatless'] == true,
      );
      ...
      if (completedMissions.isNotEmpty) {
        _showMissionUnlockedPopup(completedMissions, () {
          if (mounted) _showSuccessDialog(foodName, safeWeight);
        });
      } else {
        _showSuccessDialog(foodName, safeWeight);
      }
    } catch (e) {
      debugPrint("Manual Food Save Error: $e");
      if (!mounted) return;
      setState(() => _isSaving = false);
      // Avoid raw exception string leakage
      _showErrorSnackBar("Failed to save your meal. Please check your connection and try again.");
    }
  }
                        ...
                        TextFormField(
                          ...
                          validator: (value) {
                            ...
                            if (trimmed.length > 80) {
                              return 'Food name cannot exceed 80 characters';
                            }
                            return null;
                          },
                        ),
                  ...
                  _buildCard(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 16),
                        InkWell(
                          onTap: _isSaving ? null : _showCategoryBottomSheet,
                          borderRadius: BorderRadius.circular(16),
                          child: Container(
                      ...
                      children: [
                        ...
                        TextFormField(
                          ...
                          validator: (value) {
                            ...
                            final num? weight = double.tryParse(sanitized);
                          ...
                          Wrap(
                            ...
                            children: _ingredients.map((item) {
                              return InputChip(
                                ...
                                onDeleted: _isSaving ? null : () => _removeIngredient(item),
  ...
  Widget _buildWeightChip(String label, int weight) {
    ...
    return InkWell(
      onTap: _isSaving ? null : () => _setPresetWeight(weight),
      borderRadius: BorderRadius.circular(12),
      child: Container(
